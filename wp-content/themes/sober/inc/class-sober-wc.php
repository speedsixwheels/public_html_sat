...
class Sober_WooCommerce {
	...
	public function hooks() {
		...
		// Verify ajax add to cart calls on single product pages.
		// if ( 'yes' == get_option( 'sober_enable_single_ajax_add_to_cart' ) ) {
		// 	add_filter( 'woocommerce_add_to_cart_product_id', array( $this, 'verify_ajax_add_to_cart' ) );
		// }
	...
	/**
	 * Verify ajax add to cart call. If it fails, unset the product id.
	 *
	 * @param int $product_id
	 * @return int
	 */
	public function verify_ajax_add_to_cart( $product_id ) {
		if ( isset( $_POST['sober_add_to_cart_nonce'] ) && ! wp_verify_nonce( $_POST['sober_add_to_cart_nonce'], 'sober-add-to-cart' ) ) {
			$product_id = false;
		}
		...
		<?php if ( 'default' != $style ) : ?>
			...
			<div class="buttons-icon">
				...
				if ( 'quickview' != $style && $quickview && $quickview_behavior == 'view_button' ) {
					printf(
						'<a href="%s" class="quick_view_button button" data-product_id="%s"><svg width="20" height="29"><use xlink:href="#quickview-eye"></use></svg></a>',
	...
	public static function side_products() {
		...
		if ( 'upsell_products' == $type ) {
			$query        = new stdClass();
			$query->posts = $product->get_upsells();
